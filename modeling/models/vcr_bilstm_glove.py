...
class VCRBiLSTMGloVe(ModelBase):
  ...
  def __init__(self, model_proto, is_training):
    super(VCRBiLSTMGloVe, self).__init__(model_proto, is_training)

    if not isinstance(model_proto, model_pb2.VCRBiLSTMGloVe):
      raise ValueError('Options has to an VCRBiLSTMGloVe proto.')
