...
class VCRBiLSTMFuse(ModelBase):
  """Wraps the BiLSTM layer to solve the VCR task."""

  def __init__(self, model_proto, is_training):
    super(VCRBiLSTMFuse, self).__init__(model_proto, is_training)

    if not isinstance(model_proto, model_pb2.VCRBiLSTMFuse):
      raise ValueError('Options has to an VCRBiLSTMFuse proto.')
  ...
  def predict(self, inputs, **kwargs):
    ...
    question_feature = question_lstm_layer(question_embs,
                                           mask=question_mask,
    ...
    # Feature fusion.
    question_feature_tiled = tf.gather(tf.expand_dims(question_feature, 1),
                                       indices=[0] * NUM_CHOICES,
                                       axis=1)
    if options.text_feature == model_pb2.QUESTION_AND_ANSWER:
      answer_choices_feature = tf.concat([
          question_feature_tiled, answer_choices_feature,
          question_feature_tiled * answer_choices_feature
      ], -1)
