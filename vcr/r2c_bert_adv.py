...
def _reshape_answer_related_tensors(question_embs, question_tags, question_len,
  ...
  Args:
    question_embs: A [batch, NUM_CHOICES, max_question_len, embedding_dims] tensor.
  ...
  tile_fn = lambda x: tf.gather(tf.expand_dims(x, 1), [0] * NUM_CHOICES, axis=1)
  (question_tags, question_len) = (tile_fn(question_tags),
...
class VCRR2CBertAdv(ModelBase):
  """Wraps the BiLSTM layer to solve the VCR task."""

  def __init__(self, model_proto, is_training):
    super(VCRR2CBertAdv, self).__init__(model_proto, is_training)

    if not isinstance(model_proto, model_pb2.VCRR2CBertAdv):
      raise ValueError('Options has to be an VCRR2CBertAdv proto.')
  ...
  def _adversarial_masking(self,
    ...
    with tf.variable_scope('qa_bilinear'):
      qa_similarity = attention_ops.bilinear(
          question_embs,
          answer_embs,
          inherit_from_slim_arg_scope=options.inherit_from_slim_arg_scope)

    answer_logits = masked_ops.masked_avg(qa_similarity,
                                          mask=question_mask,
                                          dim=1)
    answer_logits = tf.squeeze(answer_logits, axis=1)

    # # Solution 1: softmax.
    a_soft_sample = masked_ops.masked_softmax(answer_logits, answer_mask, dim=1)

    tf.compat.v1.summary.histogram('masking/qa_similarity', qa_similarity)
    tf.compat.v1.summary.histogram('masking/answer_logits',
                                   tf.boolean_mask(answer_logits, answer_mask))
    tf.compat.v1.summary.histogram('masking/answer_softmax', a_soft_sample)
    ...
    batch_size = answer_logits.shape[0] // NUM_CHOICES
    mask = tf.expand_dims(1.0 - a_sample, axis=-1)

    predictions.update({
        'qa_similarity': qa_similarity,
        'answer_logits': tf.reshape(answer_logits, [batch_size, 4, -1]),
        'a_sample': a_sample,
        'a_soft_sample': tf.reshape(a_soft_sample, [batch_size, 4, -1]),
    })
  ...
  def _recognition_to_cognition(self, question_inp_features, question_len,
    ...
    with tf.variable_scope('oa_bilinear'):
      oa_similarity = attention_ops.bilinear(
          object_features,
          answer_seq_features,
          inherit_from_slim_arg_scope=options.inherit_from_slim_arg_scope)
  ...
  def _predict(self, inputs, **kwargs):
    ...
    question_embs = inputs[InputFields.question_bert]

    answer_len = inputs[InputFields.answer_choices_len]
    answer_embs = inputs[InputFields.answer_choices_bert]

    batch_size = question_embs.shape[0]
    ...
    #   with tf.variable_scope('grounding_encoder'):
    #     question_seq_features, _ = rnn.RNN(tf.stop_gradient(question_embs),
    #                                        tf.stop_gradient(question_len),
    #                                        options.rnn_config,
    #                                        is_training=is_training)

    #   with tf.variable_scope('grounding_encoder', reuse=True):
    #     answer_seq_features, _ = rnn.RNN(tf.stop_gradient(answer_embs),
    #                                      tf.stop_gradient(answer_len),
    #                                      options.rnn_config,
    #                                      is_training=is_training)

    #   answer_shortcut_mask, temperature = self._adversarial_masking(
    #       question_seq_features,
    #       tf.stop_gradient(question_len), answer_seq_features,
    #       tf.stop_gradient(answer_len), is_training, predictions)
  ...
  def build_losses(self, inputs, predictions, **kwargs):
    ...
    if options.use_sigmoid:
      loss_fn = tf.nn.sigmoid_cross_entropy_with_logits
    else:
      loss_fn = tf.nn.softmax_cross_entropy_with_logits

    labels = tf.one_hot(inputs[InputFields.answer_label], NUM_CHOICES)

    losses = loss_fn(labels=labels,
                     logits=predictions[FIELD_ANSWER_PREDICTION_ORI])
    loss = tf.reduce_mean(losses)

    losses_adv_r2c = loss_fn(
        labels=labels, logits=predictions[FIELD_ANSWER_PREDICTION_ADV_R2C])
    loss_adv_r2c = tf.reduce_mean(losses_adv_r2c)

    losses_adv_mask = loss_fn(
        labels=labels, logits=predictions[FIELD_ANSWER_PREDICTION_ADV_MASK])
