...
def _reshape_answer_related_tensors(question_embs, question_tags, question_len,
  ...
  Args:
    question_embs: A [batch, NUM_CHOICES, max_question_len, bert_hidden_units]
      float tensor.
    question_tags: A [batch, max_question_len] int tensor.
    question_len: A [batch] int tensor.
    answer_embs: A [batch, NUM_CHOICES, max_answer_len, bert_hidden_units]
      float tensor.
  ...
  question_tags = tf.tile(tf.expand_dims(question_tags, 1), [1, NUM_CHOICES, 1])
  question_len = tf.tile(tf.expand_dims(question_len, 1), [1, NUM_CHOICES])
...
class VCRR2CFrozen(ModelBase):
  """Wraps the BiLSTM layer to solve the VCR task."""

  def __init__(self, model_proto, is_training):
    super(VCRR2CFrozen, self).__init__(model_proto, is_training)

    if not isinstance(model_proto, model_pb2.VCRR2CFrozen):
      raise ValueError('Options has to be an VCRR2CFrozen proto.')
  ...
  def _predict(self, inputs, **kwargs):
    ...
    (question_embs, question_len, answer_embs, answer_len,
     answer_label) = (inputs[InputFields.question_bert],
    ...
    if options.use_max_pooling:
      pooling_fn = masked_ops.masked_max_nd
    else:
      pooling_fn = masked_ops.masked_avg_nd
    ...
    if options.use_cls_token_embedding:
      cls_bert = tf.reshape(inputs[InputFields.cls_bert],
                            [batch_size * NUM_CHOICES, -1])
      final_features = tf.concat([final_features, cls_bert], -1)
