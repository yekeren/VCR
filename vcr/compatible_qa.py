...
class VCRCompatibleQA(ModelBase):
  """Wraps the BiLSTM layer to solve the VCR task."""

  def __init__(self, model_proto, is_training):
    super(VCRCompatibleQA, self).__init__(model_proto, is_training)

    if not isinstance(model_proto, model_pb2.VCRCompatibleQA):
      raise ValueError('Options has to be an VCRCompatibleQA proto.')
