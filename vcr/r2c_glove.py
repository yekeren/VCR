...
class R2CGlove(ModelBase):
  """Wraps the BiLSTM layer to solve the VCR task."""

  def __init__(self, model_proto, is_training):
    super(R2CGlove, self).__init__(model_proto, is_training)

    if not isinstance(model_proto, model_pb2.R2CGlove):
      raise ValueError('Options has to be an R2CGlove proto.')
  ...
  def predict(self, inputs, **kwargs):
    ...
    (num_objects, object_bboxes, object_labels, object_scores, object_features,
     question_toks, question_tags, question_len, answer_toks, answer_tags,
     answer_len, answer_label) = (inputs[InputFields.num_objects],
    ...
    question_token_ids, answer_token_ids = (token_to_id_fn(question_toks),
                                            token_to_id_fn(answer_toks))
    ...
    with tf.variable_scope('question_encoder'):
      _, question_features = rnn.RNN(question_rnn_inputs,
                                     question_len,
                                     options.rnn_config,
                                     is_training=is_training)

    with tf.variable_scope('answer_choice_encoder'):
      _, answer_features = rnn.RNN(answer_rnn_inputs,
                                   answer_len,
                                   options.rnn_config,
                                   is_training=is_training)
