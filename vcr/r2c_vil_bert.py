...
class R2CVilBert(ModelBase):
  """Wraps the BiLSTM layer to solve the VCR task."""

  def __init__(self, model_proto, is_training):
    super(R2CVilBert, self).__init__(model_proto, is_training)

    if not isinstance(model_proto, model_pb2.R2CVilBert):
      raise ValueError('Options has to be an R2CVilBert proto.')
  ...
  def _predict(self, inputs, **kwargs):
    ...
    (image, height, width, num_objects, object_bboxes, object_labels,
     object_scores, object_features, question_toks, question_embs,
     question_tags, question_len, answer_toks, answer_embs, answer_tags,
     answer_len, answer_label) = (
         inputs[InputFields.img_data], inputs[InputFields.img_height],
         inputs[InputFields.img_width], inputs[InputFields.num_objects],
         inputs[InputFields.object_bboxes], inputs[InputFields.object_labels],
         inputs[InputFields.object_scores], inputs[InputFields.object_features],
         inputs[InputFields.question], inputs[InputFields.question_bert],
         inputs[InputFields.question_tag], inputs[InputFields.question_len],
    ...
    # Max objects.
    max_num_objects = tf.minimum(tf.reduce_max(num_objects),
                                 options.max_num_objects)
    ...
    question_tags = tf.where(question_tags >= max_num_objects,
                             -tf.ones_like(question_tags, tf.int32),
                             question_tags)
    answer_tags = tf.where(answer_tags >= max_num_objects,
                           -tf.ones_like(answer_tags, tf.int32), answer_tags)

    # Extract FastRCNN features.
    ...
    image_with_boxes = visualization.draw_bounding_boxes_on_image_tensors(
        tf.cast(image, tf.uint8), num_objects, object_bboxes, object_labels,
        object_scores)
    tf.summary.image('vcr/detection', image_with_boxes, max_outputs=10)

    frcnn_features, self._frcnn_init_fn = fast_rcnn.FastRCNN(
        tf.cast(image, tf.float32),
        object_bboxes,
        options=options.fast_rcnn_config,
        is_training=is_training)
    object_features = frcnn_features
  ...
  def get_scaffold(self):
    ...
    """
    return tf.train.Scaffold(init_fn=self._frcnn_init_fn)
